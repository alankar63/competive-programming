...
int main()
{ int i,j,k,te,n,q,a,b,c,d,e,f,r,s,t,m,l1,la,lc,lm,d1,da,dm,dc,x,l,am,er=0,fin,K;
ll ansp=1,anss=0,fe;
    sl(n)
    sl(k)
    sl(q)
   ...
   FOR(x,2,n+1)// for x = 2 to N:
	{  fe=(ll)(fe*(ll)(t%s))%s;
	   if(fe<=r)
       {
           A[x]=(ll)((ll)(a*power((ll)A[x-1],(ll)2,(ll)m))%m+(ll)(b*A[x-1])%m+c)%m;
       }
       else
       {
           A[x]=(ll)((ll)(d*power((ll)A[x-1],(ll)2,(ll)m))%m+(ll)(e*A[x-1])%m+f)%m;
       }
       fin=min(fin,A[x]);
    ...
    FOR(i,1,q+1)
    {
      l1 = ((ll)((la%lm) * (l1%lm))%lm + lc) % lm;
	d1 = ((ll)((da%dm) * (d1%dm))%dm + dc) % dm;
	l = l1 + 1;
	r = min(l + k - 1 + d1, n);
	//pl(l)
	//pl(r)
