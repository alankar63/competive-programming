// remember this for finding minimum in range without using a data structure
...
Weird, â??cause I don't want to see
...
I run against myself â??till I find the end.
...
ll l1,la,lc,lm,d1,da,dm,dc;
int x,l,am,er=0,fin,K,asd;
ll ansp=1,anss=0,fe,prev;
    s(n)
    s(k)
    s(q)
   ...
   FOR(x,2,n+1)// for x = 2 to N:
	{  fe=(fe*(t%s))%s;
	   ...
	   if(fe<=r)
       {
           A[x]=((((a*gh)%m)*gh)%m+(b*gh)%m+c)%m;
       }
       else
       {
           A[x]=((((d*gh)%m)*gh)%m+(e*gh)%m+f)%m;
       }
       fin=min(fin,A[x]);
	...
	if(l-1>=0 && l-1<n && r-1>=0 && r-1<n){
		 int pos=r-l-K;
		 //pl(l)
		int min2=min(ar[l],ar[l+pos]);
		anss+=(ll)min2;
		ansp=(ll)(ansp*(ll)min2)%MOD;
	}
	//anss=(anss+er);
	//ansp=(ansp*er)%MOD;
