...
void build_tree(ll node, ll a, ll b) {
 ...
 ll query_tree(ll node, ll a, ll b, ll i, ll j) {
	...
	if(a > b || a > j || b < i) return 10000000003; // Out of range
...
int main()
{ ll i,j,k,te,n,q,a,b,c,d,e,f,r,s,t,m,l1,la,lc,lm,d1,da,dm,dc,x,l,am,anss=0,ansp=1,er=0,fe;
    sl(n)
    sl(k)
    sl(q)
    ...
    sl(A[0])
   fe=t;
   FOR(x,1,n)// for x = 2 to N:
	{  fe=(fe*(t%s))%s;
	   if(fe<=r)
       {
           A[x]=(a*power(A[x-1],2,m)+b*A[x-1]+c)%m;
       }
       else
       {
           A[x]=(d*power(A[x-1],2,m)+e*A[x-1]+f)%m;
       }
    ...
    FOR(i,1,q+1)
    {
      l1 = ((la%lm) * (l1%lm) + lc) % lm;
	d1 = ((da%dm) * (d1%dm) + dc) % dm;
	l = l1 + 1;
	r = min(l + k - 1 + d1, n);
	//pl(l)
	//pl(r)
