...
{ int i,j,k,t,n,f;
   ...
   while(t--)
   {
       ...
       FOR(i,1,n)
       {
           if(s[i]=='0')// element is 0
           {
               { //for ending with 0
               if(len0[i-1]==k)//k reached
               {
                   len0[i]=1;
                  ans0[i]=ans1[i-1];
                  we0[i]=1;
               }
               else  // k not reached yet
               {

                    if(ans1[i-1]<=ans0[i-1])//checking min
                   {
                       ans0[i]=ans1[i-1];
                       len0[i]=1;
                       we0[i]=1;
               ...
               else  // k not reached yet
               {
                    ...
                    if(ans0[i-1]+1<=ans1[i-1]+1)//checking min
                   {
                       ans1[i]=ans0[i-1]+1;
                       len1[i]=1;
                       we1[i]=2;
...

           {
               ...
               else  // k not reached yet
               {
                    ...
                    if(ans1[i-1]+1<=ans0[i-1]+1)//checking min
                   {
                       ans0[i]=ans1[i-1]+1;
                       len0[i]=1;
                       we0[i]=1;
     ...
     //  k=min(ans0[n-1],ans1[n-1]);
      // p(k)
      if(ans0[n-1]<ans1[n-1])
      {
         p(ans0[n-1])
         f=0;
         for(i=n-1;i>0;i--)
