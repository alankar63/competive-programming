...
//...........................................END OF TEMPLATE.................................................................
ll a[10000001],b[10000001],c[10000001];
int main()
{ ll i,j,k,n,m,t,ans=0,p;
    sl(n)
    sl(m)
    c[0]=0;
    c[1]=1%m;
    ...
    FOR(i,2,m)
    {   c[i]=(c[i-1]*i)%m;//calculates factorial
        a[i]=(c[i]*i)%m;//calculates fact*i
        b[i]=(b[i-1]+a[i])%m;//total sum till now
        //pl(b[i])
    }
    b[0]=0;
   while(n--)
   {
      ...
      {
         ans=(((((((p%(2*m))*((p+1)%(2*m)))%(2*m))/2)*(p%(m)))%(m)) +ans)%m;
         k=((p%m)-((p/m)%m)+m)%m;
         ans=(ans+(b[k]))%m;
        // pl(ans)
      }
