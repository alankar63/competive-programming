...
char ar[200008],ch;
int main()
{ int i,j,k,t,n,ty,len,c1,c2,ans,ans1,ala,lal;
    s(t)
  fo: while(t--)
    ...
    else if(ty==1)
       {
              ...
              if(len%2==0)
          {
                FOR(i,0,len)
           {
             ...
             {
                 if(b[i]>c[i])
                    ala=ala+((b[i]-c[i])*(b[i]-c[i]+1)/2);
                 else
                    ala=ala+((c[i]-b[i])*(c[i]-b[i]+1)/2);
             }
                ...
                FOR(i,0,len)
           {
             ...
             {
                if(b[i]>c[i])
                    lal=lal+((b[i]-c[i])*(b[i]-c[i]+1)/2);
                 else
                    lal=lal+((c[i]-b[i])*(c[i]-b[i]+1)/2);
             }
             n=min(ala,lal);
             p(n)
          }
